...
public class NPETestingClient {

	private static Logger log = LoggerFactory.getLogger(NPETestingClient.class);
	...
	private static void callServerNPEinNonBlockingPingnoPong() {
		...
		// cause the server to NPE
		pingBuilder.setExtension(ExtendedPing.throwNPE, true);
	...
	private static void callServerNPEinBlockingPingnoPong() {
	...
	private static void callBlockingPingWithBlockingPongNPE() {
		BlockingPingService.BlockingInterface blockingService = BlockingPingService.newBlockingStub(channel);
		final ClientRpcController controller = channel.newRpcController();
		controller.setTimeoutMs(0);
		...
		// cause the server to NPE
		pingBuilder.setExtension(ExtendedPing.throwNPE, false);
		pingBuilder.setExtension(ExtendedPing.throwNPEonPong, true);
	...
	private static void callBlockingPingWithNonBlockingPongNPE() {
		BlockingPingService.BlockingInterface blockingService = BlockingPingService.newBlockingStub(channel);
		final ClientRpcController controller = channel.newRpcController();
		controller.setTimeoutMs(0);
		...
		pingBuilder.setPingPercentComplete(false);
		pingBuilder.setPongRequired(true);
		pingBuilder.setPongBlocking(false);
		pingBuilder.setPongDurationMs(1000);
		pingBuilder.setPongTimeoutMs(0);
		pingBuilder.setPongPercentComplete(false);
